...
# Both candidate and assessor pages share the same server = same sessions/ folder
...
with col1:
    with st.container(border=True):
        ...
        if st.button("Open Candidate Interview →", use_container_width=True, type="primary"):
            st.switch_page("pages/1_Candidate_Interview.py")
...
with col2:
    with st.container(border=True):
        ...
        if st.button("Open Assessor Review →", use_container_width=True):
            st.switch_page("pages/2_Assessor_Review.py")
